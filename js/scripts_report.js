...
function show_data_columns(department,out_id,button_id,toggle,reset) {
    var sql_args = {};
    var tables_referenced = ['employee_data'];
    var meta_sql = '';
    var preset_sql = '';
    ...
    if (CONSTANTS.DEPT_TABLES.hasOwnProperty(department)) {
        ...
        //
        tables_referenced.push(CONSTANTS.DEPT_TABLES[department]);
    }
    else {
    ...
    var callback = function(response) {
        ...
        args.tables_referenced = tables_referenced;
...
function create_production_report(parent_form_id,report_div_id,department,emp_id) {
    ...
    if (!!(document.getElementById('data_sel_cols_table'))) {
        ...
        for (var i = 0; i < all_children.length; i++) {
            ...
            if (all_children[i].id.match('-viewcol-')) {
                var obj = {};
                obj.column_name = all_children[i].value;
                col_objs[all_children[i].value] = obj;
                sel_cols.push(all_children[i].value);
            }
            if (all_children[i].id.match('-sortby-')) { name_val_obj['secd-sort'] = all_children[i].value;}
            if (all_children[i].id.match('-totaltype-')) {
                col_objs[all_children[i].name].total_type = all_children[i].value;
            }
        }
        //
    ...
    report_args.prime_sort = name_val_obj['prime-sort'].split('.')[1];
    report_args.prime_sort_dir = name_val_obj['prime-sort-dir'];
    report_args.secd_sort = name_val_obj['secd-sort'].split('.')[1];
    report_args.secd_sort_dir = name_val_obj['secd-sort-dir'];
    report_args.sect_cols = [name_val_obj['prime-sort']];
    if (report_args.prime_sort == 'emp_last_name') {report_args.sect_cols.push('emp_first_name');}
    report_args.emp_id = emp_id;
    report_args.from_ts = ts_obj.from_ts;
    report_args.to_ts = ts_obj.to_ts;
    report_args.data_sql_args = {
        'where' : [['date','BETWEEN',data_range[0]+"' AND '"+data_range[1]],
                   ['entry_status','LIKE','submitted']],
        'order_by' : [[name_val_obj['prime-sort'],name_val_obj['prime-sort-dir']],
                      [name_val_obj['secd-sort'],name_val_obj['secd-sort-dir']]]
    }
...
function make_data_sql(report_args) {
    ...
    sql_args.table = 'table_meta_data';
    sql_args.where = [
        ['in_tables','REGEXP','(^|%)employee_data(%|$)|(^|%)'+report_args.dept_table+'(%|$)'],
        ['use_on_pages','REGEXP','(^|%)report(%|$)'],
        ['use_in_html_tables','REGEXP','(^|%)report_'+report_args.department+'(%|$)']
    ];
    sql_args.order_by = [['order_index','ASC']];
    //
    sql_args.where.push(['column_name','REGEXP',report_args.sel_cols.join('$|')]);
    sql_args.where[sql_args.where.length-1][2] += '$';
...
function make_report(report_args) {
    ...
    head_row_args.tables_referenced = ['employee_data',report_args.dept_table];
